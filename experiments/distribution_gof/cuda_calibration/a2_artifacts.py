...
def run_adversarial_fixture(name, fixture, digest):
    ...
    elif name in {"nb_very_sparse","nb_heavy_tail"}:
        from types import SimpleNamespace
        import numpy as np
        from .cp05_c2c_equivalence_runner import evaluate_reference_record, evaluate_cuda_record
        from .nb_support import certify_nb_support
        sample=np.asarray(fixture["sample"],dtype=np.int64); cell=SimpleNamespace(family="negative_binomial",statistic="AD",canonical_id=name,n=len(sample))
        try:
