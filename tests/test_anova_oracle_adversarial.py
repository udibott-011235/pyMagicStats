...
def test_classical_concords_with_manual_scipy_and_statsmodels(name, groups):
    ...
    for oracle_f in (manual["f"], float(scipy_result.statistic), float(sm_raw.statistic), float(sm_summary.statistic)):
        assert result.statistic == pytest.approx(oracle_f, rel=RTOL, abs=ATOL), name
    for oracle_p in (manual["p"], float(scipy_result.pvalue), float(sm_raw.pvalue), float(sm_summary.pvalue)):
...
def test_welch_concords_with_manual_and_statsmodels(name, groups):
    ...
    means, variances, nobs = _summary_arrays(groups)
    sm_summary = anova_generic(means, variances, nobs, use_var="unequal", welch_correction=True)
...
def test_large_common_offset_preserves_results_with_adjudicated_oracles():
    groups = _scenarios()["unbalanced_k4"]
    shifted = tuple(group + 1e12 for group in groups)

    classical_baseline = OneWayANOVA(*groups, independence="assumed").run()
    classical_shifted = OneWayANOVA(*shifted, independence="assumed").run()
    scipy_classical_shifted = stats.f_oneway(*shifted)
    assert classical_shifted.statistic == pytest.approx(classical_baseline.statistic, rel=OFFSET_RTOL, abs=OFFSET_ATOL)
    assert classical_shifted.p_value == pytest.approx(classical_baseline.p_value, rel=OFFSET_RTOL, abs=OFFSET_ATOL)
    assert classical_shifted.statistic == pytest.approx(float(scipy_classical_shifted.statistic), rel=OFFSET_RTOL, abs=OFFSET_ATOL)

    welch_baseline = WelchANOVA(*groups, independence="assumed").run()
    welch_shifted = WelchANOVA(*shifted, independence="assumed").run()
    assert welch_shifted.statistic == pytest.approx(welch_baseline.statistic, rel=OFFSET_RTOL, abs=OFFSET_ATOL)
    assert welch_shifted.p_value == pytest.approx(welch_baseline.p_value, rel=OFFSET_RTOL, abs=OFFSET_ATOL)
    assert welch_shifted.denominator_df == pytest.approx(welch_baseline.denominator_df, rel=OFFSET_RTOL, abs=OFFSET_ATOL)
    ...
    if "equal_var" in inspect.signature(stats.f_oneway).parameters:
        ...
        scipy_welch_baseline = stats.f_oneway(*groups, equal_var=False)
        scipy_welch_centered = stats.f_oneway(*centered_shifted, equal_var=False)
        assert scipy_welch_centered.statistic == pytest.approx(float(scipy_welch_baseline.statistic), rel=1e-13, abs=1e-14)
        assert scipy_welch_centered.pvalue == pytest.approx(float(scipy_welch_baseline.pvalue), rel=1e-13, abs=1e-14)
        assert welch_shifted.statistic == pytest.approx(float(scipy_welch_centered.statistic), rel=OFFSET_RTOL, abs=OFFSET_ATOL)
        assert welch_shifted.p_value == pytest.approx(float(scipy_welch_centered.pvalue), rel=OFFSET_RTOL, abs=OFFSET_ATOL)
