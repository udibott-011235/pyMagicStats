...
def test_harness_is_pinned_to_the_frozen_candidate():
    assert CANDIDATE_SHA == "fb3ecc6252e8c631596b7b975e683360dcde4ae4"
    assert EXPERIMENT_VERSION == "proportion-ci-cp06-v3"
    assert HARNESS_SCHEMA_VERSION == "cp06-harness-schema-v4"
    assert ENDPOINT_CACHE_SCHEMA_VERSION == "cp06-endpoint-cache-schema-v3"
    assert SHARD_SCHEMA_VERSION == "cp06-shard-schema-v4"
...
WILSON_BOUNDARY_REGRESSION_CASES = (
    (10, 0.20, 1.0),
    (27, 0.20, 0.0),
    (29, 0.001, 1.0),
    (34, 0.05, 1.0),
...
@pytest.mark.parametrize("n", (1, 3, 27, 7_500, 100_000, 1_000_000))
def test_high_precision_wilson_boundaries_are_exact_on_small_and_stress_n(n, alpha):
    lower_at_zero, upper_at_zero = high_precision_interval(
        "wilson", n, 0, alpha, digits=80
    )
    lower_at_one, upper_at_one = high_precision_interval(
        "wilson", n, n, alpha, digits=80
    )
    assert lower_at_zero == mp.mpf(0)
    assert upper_at_one == mp.mpf(1)
    assert lower_at_zero <= mp.mpf(0) <= upper_at_zero
    assert lower_at_one <= mp.mpf(1) <= upper_at_one
    assert high_precision_binomial_runs(n, 0.0, [(0, 0)], digits=80) == mp.mpf(1)
    assert high_precision_binomial_runs(n, 1.0, [(n, n)], digits=80) == mp.mpf(1)


@pytest.mark.parametrize(("n", "alpha", "p"), WILSON_BOUNDARY_REGRESSION_CASES)
def test_wilson_boundary_regression_classification_is_dps_invariant(n, alpha, p):
    grid = production_interval_grid(n, alpha, "wilson")
    float_runs = acceptance_runs(grid.lower, grid.upper, p)
    expected_runs = [(0, 0)] if p == 0.0 else [(n, n)]
    snapshots = []
    for digits in (80, 160, 320):
        ...
        coverage = high_precision_binomial_runs(
            n,
            p,
        ...
        assert reconciliation["hp_runs"] == expected_runs
        assert not reconciliation["acceptance_changed"]
        assert coverage == mp.mpf(1)
        snapshots.append((verdict["classification"], verdict["resolved"]))
    assert snapshots == [
        ("confirmed_no_shortfall_at_audited_cell", True),
        ("confirmed_no_shortfall_at_audited_cell", True),
        ("confirmed_no_shortfall_at_audited_cell", True),
    ]
...
def test_shard_cache_rejects_legacy_and_incompatible_schema(tmp_path):
    ...
    for old_version in ("cp06-shard-schema-v2", "cp06-shard-schema-v3"):
        save_shard_cache(path, _minimal_cache_part(5), expected)
        with path.open("rb") as stream:
            payload = pickle.load(stream)
        payload["provenance"]["shard_schema_version"] = old_version
        with path.open("wb") as stream:
    ...
    with path.open("rb") as stream:
        payload = pickle.load(stream)
    payload["provenance"]["candidate_sha"] = "previous-production-candidate"
    with path.open("wb") as stream:
        pickle.dump(payload, stream)
    assert load_shard_cache(path, expected) is None
...
def test_endpoint_cache_validates_hash_provenance_and_reuses_api_grid(tmp_path):
    ...
    incompatible_metadata = {
        **metadata,
        "candidate_sha": "previous-production-candidate",
    }
    with path.open("wb") as stream:
        np.savez_compressed(
            stream,
            lower=lower,
            upper=upper,
            metadata=np.asarray(json.dumps(incompatible_metadata)),
        )
